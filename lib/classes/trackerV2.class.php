...
*/
	class TrackerV2 {
		...
		private $types;
		private $bug_status_types;
		private $bug_status_default;
		protected static $db;
		...
		public function __construct(){
			...
			//Initial Configuration for Bug Status Types, this allows for greater flexibility
			$this->bug_status_default = "green";
			$this->bug_status_types = Array("complete"=>"green",
											"incomplete"=>"red",
											"in progress"=>"yellow",
											"review"=>"purple");
		...
		public function DisplayAllBugs(){
			global $smarty;
			$query = "SELECT * FROM `trkr_bugs` LEFT JOIN (`trkr_groups`) ON (`trkr_bugs`.`bug_group` = `trkr_groups`.`group_id`) LEFT JOIN (`trkr_users`) ON (`trkr_bugs`.`bug_assignee` = `trkr_users`.`user_id`) ";
			
			//Routing glue code goes here in the future?
			if (isset($_GET['showtype'])){
				
				if (isset($this->bug_status_types[$_GET["showtype"]])){
					$query .= " WHERE bug_status = '".$_GET["showtype"]."' ";
				}
			} else {				
				if(!isset($_GET['completed'])){
				
					$query .= " WHERE bug_status != 'complete' AND bug_con = 1";
				
				}else {
				
					$query .= " WHERE bug_con = 1";
			...
			//KJS- no need to do different queries, we can join them on the relationship IDs
			//var_dump($query);
			$items = $this->db->query_as_array($query. " ORDER BY `group_name`,`bug_level` DESC,`bug_id` DESC");
			$smarty->assign("items",$items);
			$smarty->display("bug_list.tpl");					
		}
		...
		public function DisplayUniqueBug(){
			...
			$item = isset($bugs[0]) ? $bugs[0] : null;
			$smarty->assign("item",$item);
			$smarty->display("bug_individual.tpl");
		...
		public function BugStatus($bug){
		
		
			if (isset($this->bug_status_types[$bug])){
			
				return $this->bug_status_types[$bug];
			
			} else {
			
				return $this->bug_status_default;
		...
		public function DisplayCompletedTasks(){
			...
			//KJS - Good for now, will further deduplicate code later
			$tasks = $this->db->query_as_array("SELECT * FROM `trkr_bugs` LEFT JOIN (`trkr_groups`) ON (`trkr_bugs`.`bug_group` = `trkr_groups`.`group_id`) LEFT JOIN (`trkr_users`) ON (`trkr_bugs`.`bug_assignee` = `trkr_users`.`user_id`)  WHERE bug_status = 'complete'  ORDER BY `group_name`,`bug_level` DESC,`bug_id` DESC");
			$smarty->assign("items",$tasks);
			$smarty->display("bug_list.tpl");	
		...
		public function DisplayReviewable(){
			//KJS - Good for now, will further deduplicate code later
			//		(below code is very similar to the code in the function above)
			if(!isset($_GET['do'])){
			...
			$displaycategory =  Securitizor::Stripper($_GET['do']);
			
			$tasks = $this->db->query_as_array("SELECT * FROM `trkr_bugs` LEFT JOIN (`trkr_groups`) ON (`trkr_bugs`.`bug_group` = `trkr_groups`.`group_id`) LEFT JOIN (`trkr_users`) ON (`trkr_bugs`.`bug_assignee` = `trkr_users`.`user_id`)  WHERE bug_status = '".$displaycategory."'  ORDER BY `group_name`,`bug_level` DESC,`bug_id` DESC");
			$smarty->assign("items",$tasks);
			$smarty->display("bug_list.tpl");
		...
		private function EditProfile(){
			...
			$user = $this->db->q("SELECT * FROM trkr_users WHERE user_id = ". $user->id());
			$themes = $this->db->q("SELECT * FROM trkr_themes WHERE theme_status = 1");
